define(['support/ConfigurableType',
    ...
    puremvc.define({
            name: 'bmc.model.support.sectionData.delegate.EnginePriceCalculatorDelegate',
        ...
        },
        {
            calculatePrice: function () {
                ...
                var i = 0,
                    engineVO,
                    totalEngineVOs = this.dataVOs.length,
                    bodyStyleId = this.getBodyStyleId(),
                    baseVehicleProxy = this.getProxy(BaseVehiclesProxy.NAME);

                for (i; i < totalEngineVOs; i++) {

                    engineVO = this.dataVOs[i];
                    engineVO.setPrice(
                        this.getPriceOfBaseVehicleWithEngine(baseVehicleProxy, engineVO.getId(), bodyStyleId)
                    );
                }
            },
